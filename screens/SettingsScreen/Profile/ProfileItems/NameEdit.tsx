...
const NameEdit =  ({showNameEdit, setShowNameEdit}) => {
    ...
    const onPress = async () => {
        ...
        if(updatedName !== null && user?.name !== updatedName){
                await DataStore.save(
                   ...
                   ).then(setShowNameEdit(prev => !prev));
            } else {
    ...
    return(
     <>
     {showNameEdit ? <BlurView tint={"dark"} intensity={70} style={Platform.OS === "android" ?
     ...
     <BlurView intensity={150} tint={"default"} style={{padding: 15, borderRadius: 12, overflow: 'hidden',}} >
        ...
        <View >
            <TextInput maxLength={30} defaultValue={user?.name} autoFocus onChangeText={setUpdatedName} returnKeyType="done" onSubmitEditing={onPress} style={{textAlignVertical: "top"}}>
            </TextInput>
        </View>
